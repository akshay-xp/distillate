...
- ae5fea7: Add the Count-Min sketch at `distillate/countmin`: a frequency sketch that estimates how many times each key appeared, in space fixed by the error you ask for rather than by how many keys arrive.
...
- 88dc3e7: Fix `CuckooFilter.equals` returning `true` for two filters that serialize to different bytes.
...
- 5ceeeef: Add `distillate/frame` with `readFrameAt(bytes, offset)`, which validates one frame inside a larger buffer without knowing its type and returns its `byteLength`, so a buffer of concatenated frames can be walked. `fromBytes` still accepts exactly one whole frame.

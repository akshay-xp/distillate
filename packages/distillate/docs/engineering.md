...
The suite runs test files one at a time (`fileParallelism: false`) for this probe's sake. Workers competing for CPU do not make the measurement noisier, they change what it measures: under load V8 declines to inline the hash path, and `add` then really does allocate per key. Forcing the tier does not help, because `%OptimizeFunctionOnNextCall` reaches the tier but not the inlining decision, and the chain below `add` is module-private. Measured over 30 full-suite runs each: 1 failure in 30 with files in parallel, 0 in 30 without, at a cost of about 3.7 seconds.
